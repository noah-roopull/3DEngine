...
public class Screen {
  ...
  public int[] update(Camera camera,int[] pixels,double delta) {
    ...
    for (int x=0;x<width;x++) {
      ...
      int wallTexX=(int)(wallX*16);
      if (side==0&&rayDirX>0) wallTexX=16-wallTexX-1;
      if (side==1&&rayDirY<0) wallTexX=16-wallTexX-1;
      for (int y=0;y<drawStart;y++) { //ceil
        ...
        int ceilTexX=(int)((ceilX+time*10)%1*16);
        int ceilTexY=(int)((ceilY+time*10)%1*16);
        int color;
        try {
          color=ceilTex.pixels[ceilTexX+ceilTexY*16];
          ceilTexX=(int)((ceilX+time*15)%1*16);
          ceilTexY=(int)((ceilY+time*11)%1*16);
          color+=ceilTex.pixels[ceilTexX+ceilTexY*16];
        } catch (Exception e) { 
          color=0;
        }
      ...
      for (int y=drawEnd;y<height;y++) { //floor
        ...
        int floorTexX=(int)(floorX%1*16);
        int floorTexY=(int)(floorY%1*16);
        int color;
        try {
          color=floorTex.pixels[floorTexX+floorTexY*16];
      ...
      for (int y=drawStart;y<drawEnd;y++) { //walls
        int texY=(((y*2-height+lineHeight)<<6)/lineHeight)/8;
        if (wallTexX+(texY*16)<0||wallTexX+(texY*16)>255) continue;
        int color=textures[texNum].pixels[wallTexX+(texY*16)];
